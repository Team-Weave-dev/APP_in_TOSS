...
⚠️ **이 파일의 역할**: 67개 컴포넌트의 **네비게이션 가이드**

**컴포넌트 사용 전 필수**:
- 이 파일에서 컴포넌트 위치 파악
- **해당 컴포넌트 문서를 반드시 읽으세요**
- WebView 프로젝트 전용 확인

**예**: `Button` 사용 시
→ `components/button.md` **필수 참조**
...
> **이 섹션**: 모든 컴포넌트에 적용되는 범용 규칙 (상세는 링크 참조)
> **컴포넌트별 상세 사항**: 반드시 해당 컴포넌트 문서 참조

### 1. UX Writing - 컴포넌트 텍스트 (🔴 필수)

모든 컴포넌트 텍스트는 ~해요체 사용

**적용**: Button, Dialog, TextField, Toast 등 모든 텍스트
**예시**: `<Button>저장할까요?</Button>` ✅ / `<Button>저장하시겠습니까?</Button>` ❌

**참조**: [../../03-design/03-ux-writing.md](../../03-design/03-ux-writing.md)

### 2. WebView 전용 (🔴 필수)

TDS Mobile은 WebView 앱 전용 (React Native 사용 금지)

**올바름**: `react-dom` ✅ / **금지**: `react-native` ❌
